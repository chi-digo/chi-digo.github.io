  ...
  return (
    ...
    >
      ...
      <span className={styles.wordDef}>
        {locale === 'sw' ? result.equivalent_sw : locale === 'dg' ? result.equivalent_dg : result.equivalent}
      </span>
    </TrackedLink>
  );
}
...
function ProverbResultRow({ proverb, locale }: { proverb: Proverb; locale: string }) {
  return (
    <TrackedLink
      ...
      className={styles.resultRow}
    >
      <span className={styles.proverbDigo} lang="dg">{proverb.digo}</span>
...
function ArticleResultRow({ article, locale }: { article: ContentSearchResult; locale: string }) {
  const sectionLabels: Record<string, Record<string, string>> = {
    culture: { en: 'Culture', sw: 'Utamaduni', dg: 'Chimila' },
    history: { en: 'History', sw: 'Historia', dg: 'Historia' },
    language: { en: 'Language', sw: 'Lugha', dg: 'Luga' },
  };
  ...
  return (
    ...
    >
      ...
      <span className={styles.articleTitle}>{article.title[locale as 'en' | 'sw' | 'dg']}</span>
      <span className={styles.articleIntro}>{article.intro[locale as 'en' | 'sw' | 'dg']}</span>
    </TrackedLink>
  );
}
      ...
      <div className={styles.inner}>
        ...
        {q && !loading && results && (
          <p className={styles.summary}>
            {totalResults === 0
              ? (locale === 'sw' ? 'Hakuna matokeo yaliyopatikana.' : locale === 'dg' ? 'Takuna matokeo garigophahikana.' : 'No results found.')
              : `${totalResults} ${locale === 'sw' ? 'matokeo' : locale === 'dg' ? 'matokeo' : totalResults === 1 ? 'result' : 'results'}`}
          </p>
        )}
        ...
        {typeFilter && (
          <div className={styles.filterBar}>
            <span className={styles.filterLabel}>
              {locale === 'sw' ? 'Kichujio:' : locale === 'dg' ? 'Kichujio:' : 'Filter:'}
            </span>
            ...
            >
              {locale === 'sw' ? 'Ondoa kichujio' : locale === 'dg' ? 'Usa kichujio' : 'Clear filter'}
            </TrackedLink>
          </div>
        )}
        ...
        {loading && (
          <div className={styles.loading}>
            ...
            <p>{locale === 'sw' ? 'Inatafuta…' : locale === 'dg' ? 'Inatafuta…' : 'Searching…'}</p>
          </div>
        )}
