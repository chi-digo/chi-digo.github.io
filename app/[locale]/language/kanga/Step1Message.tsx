...
export function Step1Message({ proverbs, jina, jinaSource, onSetJina }: Props) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.25rem', background: 'var(--color-surface, #f5f5f5)', borderRadius: '0.5rem', padding: '0.25rem', marginBottom: '1.25rem' }}>
        <button
          ...
          style={{
            ...
            border: 'none',
            cursor: 'pointer',
            background: tab === 'proverbs' ? 'var(--color-bg, #fff)' : 'transparent',
            color: tab === 'proverbs' ? 'var(--color-bahari-accent, #2563eb)' : 'var(--fg-muted, #888)',
        ...
        <button
          ...
          style={{
            ...
            border: 'none',
            cursor: 'pointer',
            background: tab === 'custom' ? 'var(--color-bg, #fff)' : 'transparent',
            color: tab === 'custom' ? 'var(--color-bahari-accent, #2563eb)' : 'var(--fg-muted, #888)',
      ...
      {tab === 'proverbs' ? (
        ...
        <div>
          ...
          />
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.375rem', fontSize: '0.75rem', color: 'var(--fg-muted, #888)' }}>
            <span>
              {t.kanga.char_limit
      ...
      {jina.trim() && (
        <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'var(--color-surface, #f5f5f5)', borderRadius: '0.5rem', border: '1px solid var(--border, #ddd)' }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 500, color: 'var(--fg-muted, #888)', marginBottom: '0.25rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            {t.kanga.your_jina}
          </div>
          <div style={{ fontSize: '1.125rem', color: 'var(--color-bahari-accent, #2563eb)', fontFamily: 'var(--font-display, serif)' }}>
            {jina}
          </div>
        </div>
      )}
