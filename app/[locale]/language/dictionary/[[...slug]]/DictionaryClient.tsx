...
function getSenseLabel(entry: DictionaryEntry, index: number, locale: Locale): string {
  const sense = entry.senses[index];
  if (!sense) return '';
  const def = locale === 'sw' ? (sense.definition_sw || sense.definition_en)
    : locale === 'dg' ? (sense.definition_dg || sense.definition_en)
    : sense.definition_en;
  if (!def) return entry.equivalents_en?.[0] || '';
  return def.length > 40 ? def.slice(0, 40) + '…' : def;
}
        ...
        <div className={styles.wotdActions} onClick={(e) => e.stopPropagation()}>
          ...
          <ShareMenu
            ...
            sensePicker={entry.senses.length > 1 ? {
              ...
              selected: shareSense,
              onSelect: (i) => { setShareSense(i); prerenderWord(entry, locale, i); },
              getLabel: (i) => getSenseLabel(entry, i, locale),
            } : undefined}
          />
        </div>
      </div>
        ...
        <div key={sense.sense_id} className={styles.sense}>
          ...
          {(() => {
            const primary = locale === 'sw' ? sense.definition_sw
              ...
              : sense.definition_en;
            const secondary = locale === 'dg' ? null
          ...
          {sense.examples.length > 0 && (
            <div className={styles.examples}>
              {sense.examples.map((ex, i) => {
                const translation = locale === 'sw' ? ex.sw : locale === 'dg' ? null : ex.en;
                return (
            ...
            <div key={sub.id} className={styles.subEntry}>
              ...
              {(() => {
                const def = locale === 'sw' ? (sub.definition_sw || sub.definition_en)
                  : locale === 'dg' ? (sub.definition_dg || sub.definition_en)
                  : sub.definition_en;
