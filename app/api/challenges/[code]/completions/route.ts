...
export async function GET(
  ...
  let challenger: { display_name: string | null; avatar_url: string | null; score: number } = {
    display_name: null,
    avatar_url: null,
  ...
  if (challenge.challenger_id) {
    const { data: profile } = await supabase
      ...
      .single();
    if (profile) {
      challenger = { display_name: profile.display_name, avatar_url: null, score: challenge.score };
    }
  }
